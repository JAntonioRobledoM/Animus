        ...
        <div class="hologram-border bg-gradient-to-br from-black/80 to-abstergo-gray/50 rounded-xl p-8 card-glow">
            <form action="{{ route('recuerdos.update', $recuerdo->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        <div class="mb-6">
                            ...
                            <input type="text" name="title" id="title" class="w-full bg-abstergo-dark/70 border border-abstergo-blue/40 rounded py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-abstergo-blue/50" value="{{ old('title', $recuerdo->title) }}" required>
                        </div>
                        <div class="mb-6">
                            <label for="subtitle" class="block text-abstergo-accent mb-2 font-orbitron">SUBTÍTULO</label>
                            <input type="text" name="subtitle" id="subtitle" class="w-full bg-abstergo-dark/70 border border-abstergo-blue/40 rounded py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-abstergo-blue/50" value="{{ old('subtitle', $recuerdo->subtitle) }}">
                        </div>
                        <div class="mb-6">
                            <label for="year" class="block text-abstergo-accent mb-2 font-orbitron">AÑO</label>
                            <input type="number" name="year" id="year" class="w-full bg-abstergo-dark/70 border border-abstergo-blue/40 rounded py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-abstergo-blue/50" value="{{ old('year', $recuerdo->year) }}">
                        ...
                        <div class="mb-6">
                            ...
                            <select name="saga_id" id="saga_id" class="w-full bg-abstergo-dark/70 border border-abstergo-blue/40 rounded py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-abstergo-blue/50">
                                ...
                                @foreach ($sagas as $saga)
                                    <option value="{{ $saga->id }}" {{ old('saga_id', $recuerdo->saga_id) == $saga->id ? 'selected' : '' }}>
                                        {{ $saga->nombre }}
                                    </option>
                                @endforeach
                            </select>
                            <p class="text-xs text-gray-400 mt-1 font-rajdhani">Asigna este recuerdo a una saga.</p>
                        </div>
                        <div class="mb-6">
                            <label for="lugar" class="block text-abstergo-accent mb-2 font-orbitron">LUGAR</label>
                            <input type="text" name="lugar" id="lugar" class="w-full bg-abstergo-dark/70 border border-abstergo-blue/40 rounded py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-abstergo-blue/50" value="{{ old('lugar', $recuerdo->lugar) }}" placeholder="Madrid, España">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="number" name="position" id="position" class="w-full bg-abstergo-dark/70 border border-abstergo-blue/40 rounded py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-abstergo-blue/50" value="{{ old('position', $recuerdo->position) }}">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="text" name="path" id="path" class="w-full bg-abstergo-dark/70 border border-abstergo-blue/40 rounded py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-abstergo-blue/50" value="{{ old('path', $recuerdo->path) }}">
                        ...
                        <div class="mb-6">
                            <div class="flex items-center mb-3">
                                <input type="checkbox" name="necesita_app_externa" id="necesita_app_externa" value="1" class="w-4 h-4 bg-abstergo-dark/70 border border-abstergo-blue/40 rounded focus:ring-2 focus:ring-abstergo-blue/50 text-abstergo-blue" {{ old('necesita_app_externa', $recuerdo->necesita_app_externa) ? 'checked' : '' }}>
                                <label for="necesita_app_externa" class="ml-2 text-abstergo-accent font-orbitron">NECESITA APP EXTERNA</label>
                            </div>
                            <div id="ruta_app_container" class="{{ old('necesita_app_externa', $recuerdo->necesita_app_externa) ? '' : 'hidden' }}">
                                <label for="ruta_app_externa" class="block text-abstergo-accent mb-2 font-orbitron">RUTA DE APP EXTERNA</label>
                                <input type="text" name="ruta_app_externa" id="ruta_app_externa" class="w-full bg-abstergo-dark/70 border border-abstergo-blue/40 rounded py-2 px-3 text-white focus:outline-none focus:ring-2 focus:ring-abstergo-blue/50" value="{{ old('ruta_app_externa', $recuerdo->ruta_app_externa) }}" placeholder="C:\Apps\MiApp\app.exe">
                    ...
                    <div>
                        <div class="mb-6">
                            ...
                            <p class="text-xs text-gray-400 mt-1 font-rajdhani">Deja vacío para mantener la imagen actual. Formatos aceptados: JPG, PNG, GIF. Tamaño máximo: 2MB.</p>
                        </div>
                        <div class="mb-6">
                            <label class="block text-abstergo-accent mb-2 font-orbitron">IMAGEN ACTUAL</label>
                            <div class="hologram-border rounded h-40 flex items-center justify-center relative overflow-hidden" id="imagePreview">
                                @if($recuerdo->img)
                                    <img src="{{ asset($recuerdo->img) }}" alt="{{ $recuerdo->title }}" class="h-full w-auto object-contain mx-auto relative z-10">
                                ...
                                @else
                                    ...
                                    <span class="text-gray-400 font-rajdhani relative z-10">No hay imagen</span>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <div class="flex justify-between mt-8 space-x-4">
                    <a href="{{ route('recuerdos.show', $recuerdo->id) }}" class="btn-hologram text-white px-6 py-3 rounded-md transition-all duration-300 hover:bg-abstergo-gray/50 font-medium">
                        CANCELAR
                    </a>
        ...
        document.getElementById('img').addEventListener('change', function(e) {
            ...
            if (this.files && this.files[0]) {
                ...
                reader.onload = function(e) {
                    ...
                    overlay.classList.add('absolute', 'inset-0', 'bg-gradient-to-t', 'from-black/80', 'via-transparent', 'to-transparent');
                    preview.appendChild(overlay);
                }
                reader.readAsDataURL(this.files[0]);
            } else {
                @if($recuerdo->img)
                    preview.innerHTML = '<img src="{{ asset($recuerdo->img) }}" alt="{{ $recuerdo->title }}" class="h-full w-auto object-contain mx-auto relative z-10"><div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent"></div>';
                @else
                    preview.innerHTML = '<div class="absolute inset-0 bg-gradient-to-r from-transparent via-abstergo-blue/10 to-transparent animate-pulse"></div><span class="text-gray-400 font-rajdhani relative z-10">No hay imagen</span>';
                @endif
            }
        });
